...
export class MapEditorPanel {
  ...
  renderDoodads() {
    ...
    const threeQuarterTrees = MAP_EDITOR_DOODAD_CATALOG.filter((entry) => entry.perspective === "threeQuarter");
    const topDownTrees = MAP_EDITOR_DOODAD_CATALOG.filter((entry) => entry.perspective === "topDown");
    const flowers = MAP_EDITOR_DOODAD_CATALOG.filter((entry) => entry.kind === "wildflower");
    const threeQuarterPalette = this.renderDoodadPalette(threeQuarterTrees);
    const topDownPalette = this.renderDoodadPalette(topDownTrees);
    ...
    section.append(
      ...
      readout("Trees · 3/4 perspective"),
      threeQuarterPalette,
      readout("Trees · Top-down perspective"),
      topDownPalette,
      readout("Wildflowers"),
      flowerPalette,
      field("Tool", tools),
