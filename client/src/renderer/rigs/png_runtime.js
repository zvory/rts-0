...
function applyDisplayTransform(display, state, frame, part, last) {
  ...
  const next = resolvePngSpriteTransform(state, frame, part, last.next);
  if (!last.initialized || last.x !== next.x || last.y !== next.y) {
    setPoint(display.position, next.x, next.y);
  }
  if (!last.initialized || last.pivotX !== next.pivotX || last.pivotY !== next.pivotY) {
    setPoint(display.pivot, next.pivotX, next.pivotY);
  }
  if (!last.initialized || last.scaleX !== next.scaleX || last.scaleY !== next.scaleY) {
    setPoint(display.scale, next.scaleX, next.scaleY);
  }
  if (!last.initialized || last.rotation !== next.rotation) display.rotation = next.rotation;
  last.initialized = true;
  last.x = next.x;
  last.y = next.y;
  last.pivotX = next.pivotX;
  last.pivotY = next.pivotY;
  last.scaleX = next.scaleX;
  last.scaleY = next.scaleY;
