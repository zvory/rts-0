export const NEUTRAL_FRAME_STRIP_COLOR_ADJUSTMENT = deepFreeze({
  brightness: 100,
  saturation: 100,
  hue: 100,
});

export const FRAME_STRIP_TARGET_COLOR_ADJUSTMENT = deepFreeze({
  brightness: 170,
  saturation: 118,
  hue: 100,
});
...
export function isNeutralFrameStripColorAdjustment(adjustment) {
  const normalized = normalizeFrameStripColorAdjustment(adjustment);
  return (
...
export function applyFrameStripColorAdjustmentToRgba(data, adjustment) {
  const normalized = normalizeFrameStripColorAdjustment(adjustment);
  if (!data || isNeutralFrameStripColorAdjustment(normalized)) return data;
