...
connected character in each cell, and uses one scale for each authored pose sheet so the character
does not shrink as the weapon rotates. The largest pose in each group fits within six pixels of a
128×128 frame. Color is resampled with alpha-aware filtering to preserve clean transparent edges.
