...
```
The bundled `supply-300-hellhole` checkpoint is generated by
`cargo run --manifest-path server/Cargo.toml --bin generate_supply_300_hellhole`. It uses the
ordinary validated Lab spawn/command/checkpoint seams on authored `1v1`; its focused catalog test
locks exact composition and supply, partial HP, god mode, restoration, stable entity count, and
combat/projectile activity through 1,800 additional ticks.
