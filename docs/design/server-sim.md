...
| `last_world_combat_tick` | `authoritative/serialized` | Serialize the most recent tick containing hostile weapon fire or impact activity. | Snapshot projection quantizes this timestamp to identical 15-tick global activity bits and holds the signal for 60 ticks without exposing event position, owner, weapon, or count. |
...
| `last_world_combat_tick` | Optional top-level `lastWorldCombatTick`, validated not to exceed `tick`; omitted/`null` until the first hostile weapon activity. |
