...
- Replay artifact schema 3 captures the replay start state as a launch-time
  ...
  samples Scout Car, Command Car, and Tank cumulative movement oil immediately after movement but
  before combat/death cleanup. That phase placement retains vehicles that move and die on the same
  tick without adding analysis state to `GameState`, checkpoints, snapshots, or the wire protocol.
