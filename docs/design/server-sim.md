...
player resources, that Command Car's cooldown and active-sortie limit, and a completed owned City
Centre return anchor. The helper records the source Command Car, spawns the plane there, assigns the
completed owned City Centre nearest the clicked target as its return anchor, flies it to the clicked
point, starts the 10-second orbit timer only after arrival, then returns it to that City Centre and
despawns it. If the return City Centre is gone when
