...
| `entities` | `authoritative/serialized` | Serialize the full `EntityStore`, including stable entity ids, allocator/high-water state, HP, orders, queues, movement state, selected waypoints, path goals, weapon cooldowns, ability charges and charge-recharge timers, episode-keyed firing-reveal reaction gates, combat state, per-unit kill totals and last-damaging entity attribution, production/build progress, rally plans, Scout Plane source-car/orbit/remaining-lifetime state, resource reservations, body/weapon/setup facing, and entity flags. | `systems::run_tick` mutates the store every tick; snapshots, score, survival, command validation, replay determinism tests, and the Phase 0.5 comparator all treat entity state as semantic authority. Chosen movement paths and aerial orbit state live on entities, not in `pathing`. Scout Plane entities are excluded from standard fog sight stamping and contribute independent team aerial vision through the dedicated smoke-only pass. |
...
| `building_memory` | `authoritative/serialized` | Serialize remembered building entries and cleared-observation ticks per player. | `BuildingMemory::refresh` records last-seen enemy building state, while match setup seeds entity-backed neutral buildings authored into the map for every player. Hidden destroyed entries are removed only after the footprint is scouted again; that clear tick prevents older positive memory from another selected player from resurrecting the building in union observer projections. Spectator/player snapshots project remembered buildings while fogged. |
...
| `players` | `authoritative/serialized` | Serialize all `PlayerState` rows, including id/team/faction/name/color/start tile, current Steel/Oil, supply, AI flag, score counters and unit/building loss counts by kind, mined-resource lifetime totals, rolling mined-resource income history, completed upgrades, and the paused/Steel-floor/Oil-floor Auto-Build policy. | Economy, command authority, team relations, alive checks, scores, observer-analysis resource income and losses, faction-specific tech, automatic-production spending, and snapshot resource/settings rows are all read from `players`. |
...
| `building_memory` | `BuildingMemoryV1` remembered enemy-building entries and cleared-observation ticks per player, including the last-seen state and footprint facts needed for projection after restore. |
...
| `players` | `PlayerStateV1` rows with id, team id, faction id, name, color, start tile, resources, supply, AI slot flag, score counters and unit/building loss counts by kind, mined-resource lifetime totals, rolling mined-resource income history, completed upgrades, and Auto-Build pause/resource-floor settings. |
...
card to enable Artillery Fire Control behind queued Artillery research. The FIFO completion path remains the authority
...
- Build-arrival blockers are classified. A building, scaffold, resource node, terrain/out-of-bounds
  ...
  Constructed buildings spawn with 10% current HP (rounded up) and a fixed configured max HP.
  Construction linearly earns the remaining 90% over its build time: each progress update adds
  only the difference between the old and new undamaged construction HP. Damage subtracts
  directly from current HP, and reaching zero destroys the scaffold even before completion.
  Progress synchronization and completion preserve all damage; neither can revive a dead scaffold.
  Thus initial HP plus all construction gains equals exactly one full building HP budget, with
  no rounding drift or extra healing at completion. Completed buildings retain their configured
  max HP and any missing current HP. Prebuilt starting buildings begin at full HP.
...
`fired_at_tick + 13 * TICK_HZ / 2`, a fixed 6.5 seconds (195 ticks) regardless of the weapon's firing cycle.
Each later shot extends the expiry to 6.5 seconds after that shot. The fog rebuild records each source's stamped tile, whether targeting needs
...
without adding current visibility or a sight radius.
