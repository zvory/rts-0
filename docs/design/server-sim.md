...
- Authored-map schema v7 adds compact non-overlapping `[y, xStart, xEnd]` forest spans. Loading
  expands every forest tile into the existing stealth, no-vehicle, damage-reduction, and
  slow-movement vectors before `Map` construction; schema v6 remains readable.
