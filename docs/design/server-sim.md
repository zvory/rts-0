...
| `last_world_combat_tick`, `world_combat_active_through_tick` | `authoritative/serialized` | Serialize the most recent hostile-weapon activity tick and the last published 15-tick boundary through which the global signal is active. | Keeping exact observation separate from published state prevents new activity inside a quantization bucket from suppressing an already-active signal while preserving boundary-only changes. |
...
| `last_world_combat_tick`, `world_combat_active_through_tick` | Optional top-level `lastWorldCombatTick` plus `worldCombatActiveThroughTick`; the former cannot exceed `tick`, and the latter must be a bounded 15-tick publication boundary derived from observed activity. |
