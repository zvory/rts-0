...
| `entities` | `authoritative/serialized` | Serialize the full `EntityStore`, including stable entity ids, allocator/high-water state, HP, orders, queues, movement state, selected waypoints, path goals, weapon cooldowns, episode-keyed firing-reveal reaction gates, combat state, production/build progress, rally plans, Scout Plane source-car/orbit/remaining-lifetime state, resource reservations, body/weapon/setup facing, and entity flags. | `systems::run_tick` mutates the store every tick; snapshots, score, survival, command validation, replay determinism tests, and the Phase 0.5 comparator all treat entity state as semantic authority. Chosen movement paths and aerial orbit state live on entities, not in `pathing`. Scout Plane entities are excluded from standard ground-sight stamping and contribute through the every-tick aerial event layer. |
| `fog` | `authoritative/serialized` | Serialize the 15 Hz ordinary-sight base grids, current actionable grids, and bounded per-viewer firing-reveal provenance map. A later phase may reclassify fog as rebuildable only after proving an exact deterministic rebuild before every command, combat, and snapshot surface. | Scheduled `recompute_live_fog` calls rebuild ordinary moving sight; every tick restores that base and atomically stamps Scout Plane, lingering-death, and firing-reveal visibility. Combat consumes canonical reveal provenance; snapshots and commands consume the flattened actionable grid. Phase 0.5 compares per-player visible tiles as semantic state. |
...
| `lingering_sight` | `authoritative/serialized` | Serialize all active death-vision sources and their expiry ticks. | `retain_active_visibility_sources` prunes by `tick`; the every-tick event-visibility refresh stamps these sources into team fog, affecting command legality, combat visibility, and snapshots without forcing an ordinary-sight rebuild. |
| `firing_reveals` | `authoritative/serialized` | Serialize active firing-reveal sources with stable episode-start and expiry ticks. | Anti-Tank Gun and artillery/mortar reveal logic records temporary actionable sight. Repeated shots extend one continuous episode without changing its start; the every-tick event-visibility refresh stamps sources into viewer fog until expiry. |
...
player resources, that Command Car's cooldown, and its active-sortie limit. The helper records the
...
`Game` recomputes ordinary ground-unit and building sight per player every second simulation tick
(15 Hz). The held base grids make entering or leaving ordinary sight take at most one additional
30 Hz tick to become actionable. Scout Plane sight, lingering death vision, and firing reveals are
restamped over the held base every tick, so their creation and expiry remain immediate. Smoke-cloud
spawn/expiry and the creation or destruction of sight-blocking buildings wake an immediate base
rebuild because they discontinuously change occlusion. Event visibility and building-memory
refreshes use team-current views derived from the resulting actionable grids. Normal player
snapshots then build a temporary team-current fog by unioning the actionable grids of living
teammates only.
...
`game::building_memory::BuildingMemory` is server-only stale intel owned by `Game`. After the
scheduled or event-driven fog refresh, the store records one latest-seen entry per
