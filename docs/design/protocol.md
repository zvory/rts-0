...
| `cancel`     | `building: u32` | Cancel an owned unfinished building for a full construction-cost refund, or cancel the latest item in a completed building's production queue. Attached builders return to ordinary order handling when a construction site is canceled. |
