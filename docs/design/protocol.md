...
| `attackMove` | `units: u32[]`, `x: f32`, `y: f32`, `queued?: bool` | Move toward a world point while engaging enemies that enter current weapon range. The unit may pause to fire, then resumes the player-issued destination; enemy sightings never replace that path with pursuit movement. Mortar Teams move without acquiring or firing and require explicit `mortarFire` commands. Artillery treats the order as permission to spend its normal shell cost: it stops for currently team-visible enemy units in its 10-to-35-tile range band, or buildings only when no unit is eligible, sets up, and repeats minimum-radius fire while targets remain. Candidate aim points rank exposed unarmored targets (including support weapons and light cars) first, combat infantry second, and armored units last, then use a lightweight sum of enemy purchase value inside the dispersion circle within that tier; deployed guns prefer targets already inside their current firing field before redeploying. Automatic artillery fire retains ordinary friendly fire, does not coordinate overkill between guns, and resumes the original route after a one-second no-target grace. When `queued` is true, store future attack-move intent instead of replacing the active order. |
| `clearObstacleArea` | `units: u32[]`, `target: u32`, `queued?: bool` | Contextual A-click order for a completed neutral Tank Trap that is either currently team-visible or present in the issuing player's authoritative remembered-building intel. The client submits only the known trap id; the server resolves the objective center from current or remembered state and rejects never-observed ids. Units attack-move to a standable inspection point near that center, engage ordinary enemies encountered under normal attack-move rules, and dynamically give every currently actionable completed neutral Tank Trap inside the four-tile objective radius priority over ordinary targets. Hidden traps are not targeted until current vision makes them actionable. When `queued` is true, store the resolved center and objective as one future intent. There is no standalone clear-area hotkey or arbitrary ground form of this command. |
| `attack`     | `units: u32[]`, `target: u32`, `tankTrapCluster?: bool`, `queued?: bool` | Direct-target attack. Ordinary combat units keep this target locked, pursue it until reaching their current weapon range band, then stop and fire; Mortar Teams ignore the firing behavior and require explicit `mortarFire` commands. The order does not switch to a different target while its commanded target remains legal and visible. Explicit attack commands may target an enemy entity, a neutral completed Tank Trap, or an entity owned by the issuing player, allowing deliberate self-attacks on stuck friendly units/buildings. Allied teammates' entities are not valid self-attack targets, and ordinary automatic attack-move/idle acquisition remains enemy-only. The legacy `tankTrapCluster` form remains parseable for replay compatibility: when true and `target` is a visible completed neutral Tank Trap, it captures that trap plus the visible completed traps within four tiles as one atomic direct-attack list. Current clients use `clearObstacleArea` for contextual Tank Trap A-clicks. When `queued` is true, store the direct or legacy cluster attack as one future intent instead of replacing the active order. |
| `deconstruct` | `units: u32[]`, `target: u32`, `queued?: bool` | Send one selected worker to deconstruct a visible completed Tank Trap. Completed Tank Traps are neutral obstacles regardless of who built them. Deconstruction uses half of the Tank Trap build time (75 ticks / 2.5 seconds with current balance), cannot be sped up by multiple workers on the same trap, and refunds the Tank Trap cost to the deconstructing player's economy. When `queued` is true, store one future deconstruct intent using the same selected-worker allocation policy as build orders. |
| `setupAntiTankGuns` | `units: u32[]`, `x: f32`, `y: f32`, `queued?: bool` | Manually emplace owned Anti-Tank Guns and Artillery toward the submitted world point. When `queued` is true, append a future setup intent for owned completed setup-capable support weapons only; the stored facing point is evaluated from the unit's position when the stage promotes. Immediate setup clears movement/target state and begins after alignment. Mortar Teams and other selected units are ignored. |
| `tearDownAntiTankGuns` | `units: u32[]` | Pack up owned Anti-Tank Guns and Artillery that are `setting_up` or `deployed`, or cancel their staged setup facing while packed. Mortar Teams and other selected units are ignored. |
...
| `artilleryFire` | `units: u32[]`, `x: f32`, `y: f32`, `radiusTiles: f32`, `queued?: bool` | Unified Artillery Fire command. `x`/`y` is the selected center and `radiusTiles` is the click-click or press-drag-release radius. The server clamps it to 6–15 tiles normally or 3–15 after Artillery Fire Control. Each gun preserves the clicked center: a gun already inside its 10-to-35-tile range band sets up or redeploys in place, while an out-of-range gun packs up, paths to a legal firing position, sets up facing the center, and then begins its terminal repeating fire order. Every shell lands at a uniformly area-sampled point inside the selected circle; there is no additional accuracy error or repeated-shot tightening. |
| `useAbility` | `ability: "charge"|"smoke"|"mortarFire"|"pointFire"|"blanketFire"|"breakthrough"|"scoutPlane"|"dismissScoutPlane"|"ekatTeleport"|"ekatLineShot"|"ekatMagicAnchor"|"ekatConsumeGolem"|"barrage"`, `units: u32[]`, `x?: f32`, `y?: f32`, `queued?: bool` | Generic ability command. Ability ids, carriers, target mode, cost, cooldown, finite uses, queueability, queue policy, autocast support, command-card exposure, and upgrade requirements are mirrored from the Rust faction ability registry. `charge` is legacy/no-op with no carriers or runtime effect; `dismissScoutPlane` is reserved legacy vocabulary with no carriers or command-card descriptor. `smoke`, `scoutPlane`, `mortarFire`, `barrage`, Ekat `ekatTeleport`, `ekatLineShot`, and `ekatMagicAnchor` target a world point. Rocket Truck `barrage` requires Rockets research, accepts in-range world points from 10 through 44 tiles without a current-vision requirement, stops and turns the vehicle toward the target without discarding the command, then schedules exactly 16 scattered rocket impacts over a four-second unload and ends; its cooldown lasts 60 seconds from activation; the truck's first barrage is free and later barrages cost 150 oil. Command Car `breakthrough` and Ekat `ekatConsumeGolem` are self-targeted and ignore `x`/`y`. After `scout_plane_unlock` completes, `scoutPlane` launches from a selected ready Resource Depot for 38 steel / 56 oil or Command Car for 63 steel / 94 oil, starts that carrier's 30-second cooldown, and gives the independent sortie a 30-second total lifetime from launch; transit consumes that lifetime, the plane orbits only for any time remaining after arrival, and it despawns on expiry even if it never reaches the clicked point. Any number of valid sorties may coexist, and every active plane contributes its own aerial vision. Smoke command execution is phased separately from the authoritative smoke world-state/LOS model; mortar fire and Rocket Truck barrage schedule delayed area impacts. Registry queue policy differentiates non-queued, skip-if-not-ready, and wait-until-ready abilities; queued Mortar Fire and Barrage wait until ready and then fire once per queued click. Legacy Artillery `pointFire` and `blanketFire` ids remain decodable for older commands and replays, but the live command card emits `artilleryFire`. Ekat dash and line shot clamp out-of-range world targets to their max range instead of walking Ekat into range; queued Ekat ability commands append future dash, line shot, or Magic Anchor intents. Ekat dash moves her within the target range if the landing point is statically standable and leaves an authoritative return marker at the original position; Ekat line shot spawns a projected out-and-back line projectile that damages enemy targetables on each swept leg, and an active Magic Anchor adds a second projectile from the anchor toward the same point; Magic Anchor places one replacement-style, non-blocking, non-attackable 10-second pull field. Ekat Consume is non-queued; it permanently removes the nearest owned living Golem within 2 tiles and heals Ekat to full HP. |
...
| `setAutocast` | `ability: string`, `units: u32[]`, `enabled: bool` | Legacy parseable command. No current ability supports autocast, so all requests are ignored. |
| `gather`     | `units: u32[]`, `node: u32`, `queued?: bool` | Send faction gather-capable units to a live steel node. Kriegsia Engineers are no longer gather-capable; Ekat Golems retain direct Steel gathering. A legal gatherer may travel to a patch and wait for mining-anchor coverage. Oil nodes remain non-direct-mineable. When `queued` is true, store future gather intent instead of replacing the active order. |
| `build`      | `units: u32[]`, `building: string`, `tileX: u32`, `tileY: u32`, `queued?: bool` | Selected workers construct a building at a tile. The server allocates one compatible worker per build click, first walks that worker to a nearby point outside the requested footprint, then starts construction once it is in range. `building` ∈ worker-buildable building kinds. Steel Mines and Pump Jacks are excluded because Resource Depots create them through `train`. When `queued` is true, store future build intent instead of replacing the active order. |
| `train`      | `building: u32`, `unit: string` | Queue one manual production item. Most items are units; a Resource Depot also accepts `steel_mine` and `pump_jack`. A paid front extractor item immediately creates an under-construction scaffold on the nearest deterministic live, unoccupied matching resource patch within that producer's mining range. Its public `buildProgress` equals the depot item's `prodProgress`, and completing the queue item completes that same scaffold. An extractor waits unpaid at zero progress while no eligible patch exists. Other legal manual requests may wait for Steel, Oil, or supply. Later FIFO items do not prepay. |
| `adjustProductionRepeat` | `buildings: u32[]`, `unit: string`, `delta: -1\|1` | Atomically adjust a production item across selected compatible producers. `+1` enables it on one producer that does not already repeat it, preferring the producer with the fewest standing repeat items; `-1` removes it from one producer that repeats it, preferring the producer with the most standing repeat items so another automatic order survives when possible. Stable opposite entity-id tie-breaks make repeated additions followed by removals reversible. When a producer's ordinary queue is empty, it silently retries the ordered list's current eligible entry. Entries missing a completed building or upgrade prerequisite stay enabled but are skipped until unlocked; resource, supply, pause, and reserve failures wait without skipping the selected eligible entry. A repeated Steel Mine or Pump Jack creates no queue item and spends nothing while every matching patch in that producer's range is occupied; destroying an extractor makes the repeat eligible again. Each successful automatic enqueue advances to the next enabled item. Any production cancel clears the affected producer's whole repeat list. |
| `setAutoBuildSettings` | `paused: bool`, `reserveSteel: u32`, `reserveOil: u32` | Replace the issuing player's authoritative Auto-Build policy. New players start unpaused with both resource floors at 0. The server clamps each reserve to 0–9,950. While paused, standing repeat orders do not enqueue new units. Otherwise, for each resource an automatic item actually spends, the resulting stockpile must stay at or above that resource's floor. A zero-cost resource does not gate the item, so Worker Auto-Build can run below the Oil floor without spending Oil. Already-queued and manual production remain ordinary FIFO work and are unaffected. |
| `research`   | `building: u32`, `upgrade: string` | Queue a permanent player upgrade at a tech building, including a valid Training Centre or Engineering Complex that is still under construction. Research queued during construction remains unpaid and at zero progress until the building completes. A legal manual request may also wait unpaid at zero progress until its cost is available. A prerequisite must either be complete or already appear earlier in the same building's authoritative FIFO research queue. Current Kriegsia upgrade ids: `methamphetamines`, `panzerfausts`, and `entrenchment` at the Training Centre; `artillery_unlock` (Artillery), `ballistic_tables` (Artillery Fire Control), `tank_unlock`, `smoke_plus`, `scout_plane_unlock` (Scout Plane), and `rockets` (Rockets) at the Engineering Complex (`engineering_complex`). `panzerfausts` unlocks Panzerfaust training at the Barracks without changing Riflemen. Anti-Tank Guns need no research and are immediately trainable from a completed Gun Works. `anti_tank_gun_unlock` remains decodable for legacy commands and replays but is not researchable. `artillery_unlock` independently unlocks Artillery training; `ballistic_tables` requires `artillery_unlock` and reduces the Artillery Fire minimum radius from 6 tiles to 3; `tank_unlock` unlocks Tank training. `smoke_plus` doubles Scout Car Smoke radius and duration. `scout_plane_unlock` unlocks the Resource Depot and Command Car Scout Plane ability. `rockets` costs 75 steel and 125 oil, takes 25 seconds, and unlocks Rocket Truck production at Gun Works. |
| `cancel`     | `building: u32`, `construction?: bool` | With `construction: true`, cancel an owned unfinished building for a full construction-cost refund; an extractor scaffold instead cancels and refunds its owning Resource Depot item. Otherwise cancel the latest item in a completed building's production queue; cancelling its active extractor item also removes the linked scaffold. The explicit construction scope prevents a delayed scaffold action from cancelling production after the building completes. Attached builders return to ordinary order handling when a construction site is canceled. |
...
| `groundDecals` | `requestId: u32`, `revision: u32`, `decals: GroundDecalView[]`, `tankTrails: TankTrailView[]` — reliable fog/observer-scoped delta response. `requestId` echoes the request so stale pre-seek or pre-view-change responses cannot enter the current cache. A trail row carries an immutable chunk `id` and sparse `[xQuarterPx, yQuarterPx, headingI16]` poses; these wire units remain stable even though historical poses are deliberately coarsened before projection. The server discloses a historical chunk when its indexed swept bounds first intersect current team vision. As an explicit best-effort-cosmetic exception to positional fog secrecy, the client may therefore hold nearby hidden tread geometry from that bounded chunk; no live entity or command state is included. It reconstructs and permanently rasterizes the approximate swept left/right tread footprint only inside individual currently visible map tiles. Ordinary decal rows carry `id`, `decalClass`, `sourceKind`, `x`, `y`, `owner`, `seed`, and optional `facing`, `weaponFacing`, or `radiusTiles`. |
...
record layouts remain safe to decode. Version 56 appends optional entity slot 43,
`rocketRackCount` (0–16). This public physical rack state is included only on already-visible
Rocket Trucks; it reveals neither a private reload timer nor a hidden unit. Sixteen PNG rounds
remain attached while moving, disappear individually in the server launch order, and return when
the existing reload cooldown reaches zero. `mortarLaunch.fromX/fromY` for rockets are the exact
rotated rack slot centres, not the truck centre. Launch visibility remains owner/team-only.
The client retains the visible shooter owner/facing with each launch for detached team-tinted
projectiles, without inventing or exposing hidden targets.
...
stages carry the server-stored clicked center; the
...
Scout Car `smoke` remains visible but disabled until the owner has a completed Engineering Complex. The
server checks that building requirement when a Smoke command is issued and again when a queued
Smoke order promotes, so losing the Engineering Complex relocks the ability.
...
`autocastEnabled` is reserved for future registry-backed abilities; no current ability projects it.
...
snapshot version 53. Versions 51 and 52 omit that slot and decode the field as absent. Compact
version 54 additionally permits a trailing `rocket: true` style bit on mortar launch and impact
records; rocket impacts retain null attacker/reveal placeholders so the style bit always occupies
its unambiguous trailing slot. The decoder also accepts the brief placeholder-free version-54
shape. The bit changes presentation only, while damage and fog remain authoritative.
...
not become target ids. Its selected radius and server-authoritative clicked center do not make
...
remembered footprint is observed empty while another player's stale positive memory remains. A
selected player's clear at or after the newest positive observation suppresses that building from
the union even after current vision leaves, preventing older selected-player memory from
resurrecting destroyed buildings. Clears with no remaining positive memory are discarded. Clear
ticks remain internal; the wire continues to carry only positive `RememberedBuilding` rows. This
keeps one-player replay vision isolated to that player's memory without adding a memory-source wire
field.
...
```
{
  ...
  rocketRackCount?: u8,           // 0–16 physical rounds; visible Rocket Trucks only
  ...
  setupState?: string,           // machine_gunner/anti_tank_gun/artillery only:
  ...
  abilities?: [                  // owner-only ability affordance/cooldown data
    { ability: "smoke"|"mortarFire"|"pointFire"|"blanketFire"|"breakthrough"|"scoutPlane"|"dismissScoutPlane"|"ekatTeleport"|"ekatLineShot"|"ekatMagicAnchor"|"ekatConsumeGolem"|"barrage",
...
{ e: "mortarLaunch", from: u32, fromX: f32, fromY: f32, toX: f32, toY: f32, radiusTiles: f32, delayTicks: u32, rocket?: bool }
...
replayed to viewers; each sample is a complete authoritative world state. Recorded 1,000-tick
