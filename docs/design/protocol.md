...
| `requestGroundDecals` | `afterRevision: u32` | Request reliable durable ground marks learned after this recipient-scoped cursor. Accepted only during an active live/replay simulation and rate-limited to one response per connection per 500 ms. |
...
| `groundDecals` | `revision: u32`, `decals: GroundDecalView[]` — reliable fog/observer-scoped delta response. Each row carries `id`, `decalClass`, `sourceKind`, `x`, `y`, `owner`, `seed`, and optional `facing`, `weaponFacing`, or `radiusTiles`. |
