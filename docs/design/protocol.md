...
| `research`   | `building: u32`, `upgrade: string` | Queue a permanent player upgrade at a tech building. A legal manual request may wait unpaid at zero progress until its cost is available. A prerequisite must either be complete or already appear earlier in the same building's authoritative FIFO research queue. Current Kriegsia upgrade ids: `methamphetamines`, `panzerfausts`, and `entrenchment` at the Training Centre; `anti_tank_gun_unlock` (Medium Guns), `artillery_unlock` (Heavy Guns), `tank_unlock`, `mortar_autocast`, and `smoke_plus` at the R&D Complex (`research_complex`). `panzerfausts` unlocks Panzerfaust training at the Barracks without changing Riflemen. `anti_tank_gun_unlock` unlocks Anti-Tank Gun training; `artillery_unlock` requires `anti_tank_gun_unlock` and unlocks Artillery training; `tank_unlock` unlocks Tank and Command Car training. `smoke_plus` doubles Scout Car Smoke radius and duration. Compact upgrade code 7 remains reserved for legacy `ballistic_tables` data but the upgrade is not researchable. |
