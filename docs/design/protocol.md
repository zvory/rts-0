...
| `setTeamPreset` | `preset: "solo"|"ffa"|"1v2"|"1v3"|"2v2"` | Host selects a lobby team preset (lobby phase only, host-only). `ffa` is the default, and the normal lobby UI exposes `solo`, `1v2`, `1v3`, and `2v2` after the team-safety gates. |
...
| `useAbility` | `ability: "charge"|"smoke"|"mortarFire"|"pointFire"|"breakthrough"|"ekaterinaTeleport"|"ekaterinaLineShot"`, `units: u32[]`, `x?: f32`, `y?: f32`, `queued?: bool` | Generic ability command. Ability ids, carriers, target mode, cost, cooldown, finite uses, queueability, autocast support, and command-card exposure are mirrored from the Rust faction ability registry. `charge` is legacy/no-op; `smoke`, `mortarFire`, deployed Artillery `pointFire`, Ekaterina `ekaterinaTeleport`, and Ekaterina `ekaterinaLineShot` target a world point. Command Car `breakthrough` is self-targeted and ignores `x`/`y`. Smoke command execution is phased separately from the authoritative smoke world-state/LOS model; mortar fire schedules a delayed area impact. Artillery point fire requires a deployed gun and is terminal in the unit order queue: once accepted, later queued unit orders are not appended after it. Ekaterina teleport moves her within the target range if the landing point is statically standable; Ekaterina line shot applies immediate line damage to enemy targetables. |
...
The canonical default faction id is `kriegsia`; `ekaterina` is also a playable catalog id. Start payloads emit `factionId` for every active
