...
| `research`   | `building: u32`, `upgrade: string` | Queue a permanent player upgrade at a tech building. Current Kriegsia upgrade ids: `methamphetamines` at the Training Centre; `anti_tank_gun_unlock` (Field Ordnance), `ballistic_tables`, `tank_unlock`, `command_car_unlock`, and `mortar_autocast` at the R&D Complex (`research_complex`). `anti_tank_gun_unlock` unlocks both Anti-Tank Gun and Artillery training. `ballistic_tables` requires completed `anti_tank_gun_unlock`; `command_car_unlock` requires completed `tank_unlock`. Legacy `artillery_unlock` remains in the compact upgrade vocabulary for old snapshot/replay decoding but is not current-faction research. |
