...
trail excludes road-tile pixels so paved surfaces never retain tank tracks. Historical road-only
tile buckets are discarded before rasterization; live cross-boundary stamps clear their road area.
...
for those rooms. Paused replay viewers can advance exactly one simulation tick with the shared step
control. Replay viewers also get a local `Hide end time` checkbox that removes the total
...
from `window.location`, and shows `Lobby`; after the server confirms entry into a live or replay
lobby it prepares the compatible renderer behind the still-visible lobby. During a normal
multiplayer `matchCountdown` it sends `matchLoadReady` only
...
a compatible live or replay match atomically adopts it, while Lab, failed, or superseded paths
destroy it and await teardown. Ownership then transfers to `Match`, which remains responsible
...
Ground decal rendering (`state_ground_decals.js`, `renderer/decals.js`; durable layer `decals`
between terrain and trenches, transient corpse layer `corpses` immediately above trenches):
...
- Moving or pivoting tanks leave paired tread marks. Every currently visible tank paints precise
  ...
  pivots. Marks use a faint translucent earth tint and exclude road tiles for both live and
  later-discovered history; known road-only history is skipped before rasterization. Separate
  512-world-pixel tread tiles avoid repeatedly uploading the whole-map
...
- Units: SVG-authored rigs remain authoritative for Worker, Golem, and Ekat. Production PNG strips
  ...
  unit occupies an authored concealment tile, or its readability footprint intersects a foreground
  concealment tile, the renderer redraws that unit's current production
  ...
  respective owner colors. Decorative tree PNG overlap alone does not trigger the outline, so the
  cue remains synchronized with the map's authoritative concealment mask. Authoritative `visionOnly` concealment reveals
