...
the current preset. The legacy host-only `setQuickstart` compatibility command still exists only as
a temporary deletion target; active tests and the normal lobby must not use it. Use lab rooms for
player-facing experimentation until debug-style starts return as explicit lab presets or scenarios.
