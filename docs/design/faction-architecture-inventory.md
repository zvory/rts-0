...
- Units: Worker, Golem, Rifleman, Machine Gunner, Panzerfaust, Anti-Tank Gun, Mortar Team,
  Artillery, Scout Car, Tank, Command Car, and Ekat.
...
Mortar Teams immediately, Anti-Tank Guns after Heavy Guns research, and Artillery after Heavy
Guns research.
...
| Replay playback | `ReplayArtifactV1.players[].faction_id` and `playerLoadouts[]` in artifact schemas 2/3 | Recorded playable ids `kriegsia` or `ekat`; missing, unknown, and fixture ids reject | From artifact only | Disabled for replay viewers | Schema 3 restores a checkpoint-backed start state; schema 2 loads from map/seed/loadouts; never lobby state | `server/crates/sim/src/game/replay.rs` tests, `server/src/lobby/room_task.rs` replay tests |
