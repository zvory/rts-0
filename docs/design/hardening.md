...
- **Chat bounds and authority** (`room_task/chat.rs`, `replay_session.rs`): the server collapses
  whitespace, caps accepted text at 200 Unicode scalar values, drops empty messages, and admits at
  most five messages per sender per rolling ten seconds. It derives names, teams, phase, replay
  tick, and recipients from room state rather than client fields. Live replay capture is capped at
  10,000 entries, and replay validation repeats entry count, tick ordering/range, name, and text
  bounds before playback.
...
- **Static map doodad bounds** (`rts-contract`, `game/map/authored.rs`, `map_handoffs.rs`, and Lab
  ...
  fields. Schema-v8 forest spans require in-bounds inclusive x ranges and reject overlap; their
...
- **Tank body and weapon facing**: the snapshot `facing` field is the tank hull/body angle. Tanks
  ...
  Packed Anti-Tank Guns use the same pivot-drive policy with a separate exact 55-degree-per-second
  body cap (`ANTI_TANK_GUN_BODY_TURN_RATE_DEGREES_PER_SECOND = 55`).
...
- **Methamphetamines research**: Training Centres can queue one permanent player upgrade costing
  ...
  status.
- **Advanced research locks**: Anti-Tank Guns are immediately trainable from a completed Gun Works;
  `anti_tank_gun_unlock` remains protocol-decodable only for legacy replays and is not researchable.
  Engineering Complex can queue `artillery_unlock` for 200 steel / 100 oil over 750 ticks,
  unlocking Artillery training at Gun Works;
...
- **Route waypoint skipping**: intermediate movement waypoints are path-following hints, not hard
  ...
  from its current position. Infantry and workers use the same standability segment check, with
  their circular bodies, so they can skip reachable dog-leg points instead of oscillating around the
  waypoint center. Blocked building corners still keep the intermediate waypoint until the swept
  segment is legal. Oriented vehicles
