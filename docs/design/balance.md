...
| Fog overlay alpha | Authoritative fog visibility is in sim snapshots; alpha is not a Rust value | `client/src/config.js` `MAIN_MAP_FOG_*` (`0.30` explored, `0.60` unexplored) and `MINIMAP_FOG_*` (`0.48` explored, `0.80` unexplored); stable `FOG_*` aliases retain the minimap values | UI-only presentation data | Client config contracts pin the separate viewport/minimap values | Exclusion list in future config parity check | Client owns opacity; Rust owns which tiles/entities are visible. Main-map fog is intentionally lighter so terrain and roads remain legible, while minimap tuning remains unchanged. | No compact impact |
