...
structured parity gap is command budget, which is sim command-service policy mirrored by the client
outside the Rust rules dump.
...
| Human command budget values `BASE_COMMAND_SUPPLY_CAP`, `COMMAND_CAR_SUPPLY_CAP_BONUS` | `server/crates/sim/src/game/services/commands.rs`, owned by command admission policy | `client/src/config.js` exports consumed by `client/src/command_budget.js`, selection, control groups, and HUD budget display | sim command-service policy mirrored by client | Server command-budget tests and client contract tests exercise each side independently; no structured Rust-to-JS parity yet | Phase 2 should add a sim-owned structured comparison and config export-name snapshot before client config splits | Not client-only because the client mirror previews the server's command acceptance cap; not Rust rules-owned unless a later design explicitly moves ownership | No compact impact |
