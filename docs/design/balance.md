...
- **Artillery** (Engineering Complex research, protocol id `artillery_unlock`): costs 200 steel / 100 oil
  and takes 750 ticks (~25s). It has its own permanent Engineering Complex slot and can be researched
  independently. The former AT Guns slot before it remains empty so all other research hotkeys retain
  their established positions. Once complete, that player can train Artillery from Gun Works.
...
| anti_tank_gun         | 45  | 100 deployed; 30 vs infantry-sized targets | 20 deployed | 108 | 1.672 | 9    | 150 | 40  | 6   | 440 (~15s); cannot fire while packed or transitioning; available immediately from a completed Gun Works (`steelworks` kind) |
