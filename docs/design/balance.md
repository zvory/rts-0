...
- **Rockets** (Engineering Complex research, protocol id `rockets`): costs 75 steel / 125 oil and
  takes 750 ticks (~25s). Once complete, that player can train Rocket Trucks from Gun Works.
...
| rocket_launcher (Rocket Truck) | 150 | 16 rockets, each 40 outer / 100 inner AOE | 10-35 Barrage | 450-tick (~15s) cooldown from activation | 2.0 | 8 | 225 | 125 | 6 | 600 (~20s); requires Gun Works (`steelworks` kind) and Rockets (`rockets`) researched in Engineering Complex; vehicle movement; must stop to launch; one manual command unloads exactly 16 rockets over 60 ticks (~2s) into a 4-tile scatter radius and then stops; the rack tubes darken during the cooldown; Barrage accepts in-range world points without requiring current vision; first barrage is free and later barrages cost 75 oil |
