...
- Scout Plane constants for the Command Car Scout Plane ability:
  ...
  instantly from the selected Command Car, and admits at most one active Scout Plane per Command
  Car. Each Command Car tracks its own 30-second cooldown.
...
| scout_plane     | 40  | 0   | 0     | 0  | 2.0   | 15    | 50  | 50  | 0   | 0; launched instantly from the selected Command Car and returns to the owned completed City Centre nearest the target; one active per Command Car; non-combat recon with 4-tile orbit radius, 10-second orbit after arrival, 30-second caster-local cooldown, no ground collision reservation, and 48x34 px client render body |
