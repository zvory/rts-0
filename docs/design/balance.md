...
| Fog overlay alpha | Authoritative fog visibility is in sim snapshots; alpha is not a Rust value | `client/src/config.js` `MAIN_MAP_FOG_*` (`0.30` explored, `0.60` unexplored) and `MINIMAP_FOG_*` (`0.48` explored, `0.80` unexplored) | UI-only presentation data | Client config contracts pin the separate viewport/minimap values | Exclusion list in future config parity check | Client owns opacity; Rust owns which tiles/entities are visible. Main-map fog is intentionally lighter so terrain and roads remain legible, while minimap tuning remains unchanged. | No compact impact |
...
excluded from ordinary automatic acquisition for every unit and weapon, including ground Attack
Move and route obstruction. A contextual A-click on a visible or remembered completed Tank Trap
creates a four-tile clear-area objective: units retain normal Attack Move engagement against enemies
...
- **Rockets** (Engineering Complex research, protocol id `rockets`): costs 75 steel / 125 oil and
  takes 750 ticks (~25s). Once complete, that player can train Rocket Launchers from Gun Works.
...
| rocket_launcher | 150 | 16 rockets, each 40 outer / 100 inner AOE | 10-35 Barrage | 210 total barrage cycle | 2.0 | 8 | 225 | 125 | 6 | 600 (~20s); requires Gun Works (`steelworks` kind) and Rockets (`rockets`) researched in Engineering Complex; vehicle movement; must stop to launch; unloads over 60 ticks (~2s) into a 4-tile scatter radius, then reloads for 150 ticks (~5s); first barrage is free and later barrages cost 75 oil |
...
| engineering_complex           | Engineering Complex        | 165 | 1     | 100 steel + 100 oil | 3x3  | 450       | research-only building for AT Guns, Artillery, Rockets, Tank Production, Smoke Plus, and Scout Plane; requires a Resource Depot and Training Centre |
...
| steelworks                 | Gun Works          | 200 | 1     | 150 steel + 100 oil | 3x3  | 599       | Superior Firepower path building; trains mortar_team immediately, Anti-Tank Guns after AT Guns, Artillery after Artillery research, and Rocket Launchers after Rockets research; requires a Resource Depot and Training Centre |
