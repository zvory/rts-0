...
- No combat air forces. `scout_plane` is non-combat reconnaissance launched by the Command Car
  Scout Plane ability. Each sortie flies to a targeted orbit, provides vision for 20 seconds after
  arrival, and then disappears.
...
- Scout Plane constants for the Command Car Scout Plane ability:
  ...
  `SCOUT_PLANE_ORBIT_RADIUS_TILES = 4`, `SCOUT_PLANE_ORBIT_DURATION_TICKS = 600`,
  ...
  without a City Centre requirement and admits at most one active Scout Plane per Command Car.
  Each Command Car tracks its own 30-second cooldown.
