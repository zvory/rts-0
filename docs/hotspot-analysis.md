...
| `balance-and-config` | `server/crates/rules/src/balance.rs`, future `server/crates/rules/src/balance/**`, `server/crates/rules/src/defs.rs`, `server/crates/rules/src/faction.rs`, `server/src/config.rs`, `server/crates/sim/src/config.rs`, `client/src/config.js`, future `client/src/config/**`, future `client/src/config_*.js`, wiki/faction parity scripts | Rust rules are authoritative and the client config mirror is player-visible; split files must still be reviewed as one balance mirror. |
