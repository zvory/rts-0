...
import {
  InteractError, InteractService, conciseError, loadDriverFactory, normalizeError,
...
import { InteractTailnetPreview } from "./tailnet_preview.ts";
...
export async function runDaemon({ workspaceRoot = process.cwd(), idleMs = configuredIdleMs(), startupNonce = "" } = {}) {
  ...
  if (!/^[a-f0-9]{32}$/.test(startupNonce)) throw new Error("Interact daemon requires a valid startup nonce.");
  claimStartupLock(paths, startupNonce);
  const prebindDelayMs = Number(process.env.RTS_INTERACT_TEST_PREBIND_DELAY_MS || 0);
  ...
  if (!startupLockOwned(paths, startupNonce, process.pid, "daemon")) {
    throw new Error("Interact startup lease was replaced before the daemon could bind.");
  }
  const driverFactory = await loadDriverFactory(paths.workspaceRoot);
  let artifactPreview: InteractTailnetPreview;
  ...
  const configuredCheckoutCommit = process.env.RTS_INTERACT_TEST_CHECKOUT_COMMIT || "";
  if (configuredCheckoutCommit && !/^[a-f0-9]{40}$/.test(configuredCheckoutCommit)) {
    throw new Error("RTS_INTERACT_TEST_CHECKOUT_COMMIT must be a lowercase 40-character Git SHA.");
  }
  const daemonCheckoutCommit = configuredCheckoutCommit || checkoutCommit(paths.workspaceRoot);
  const omitCheckoutMetadata = process.env.RTS_INTERACT_TEST_OMIT_CHECKOUT === "1";
  ...
  artifactPreview = new InteractTailnetPreview({ workspaceRoot: paths.workspaceRoot });
  ...
  const handle = async (line: string, socket: net.Socket) => {
    ...
    try {
      ...
      if (Object.keys(request).length === 2 && request.protocolVersion === IPC_VERSION && request.probe === "interact") {
      ...
      if (shutdownRequested) throw Object.assign(new Error("Interact is already shutting down."), { code: "serviceClosed" });
      if (request.refreshCheckout != null) {
        ...
        if (activeRequests !== 0 || !service.canRefreshCheckout()) {
          throw new InteractError(
            ...
            "The Interact daemon belongs to another checkout commit and has active work. The scene was preserved; inspect status, then explicitly shut down when it is safe to discard.",
            {
  ...
  await new Promise<void>((resolve, reject) => {
    if (!startupLockOwned(paths, startupNonce, process.pid, "daemon")) {
      reject(new Error("Interact startup lease was replaced before socket bind."));
      return;
    }
  ...
  try {
    ...
    const startupDelayMs = Number(process.env.RTS_INTERACT_TEST_STARTUP_DELAY_MS || 0);
    if (Number.isInteger(startupDelayMs) && startupDelayMs > 0 && startupDelayMs <= 2_000) {
      await new Promise((resolve) => setTimeout(resolve, startupDelayMs));
    }
    if (!startupLockOwned(paths, startupNonce, process.pid, "daemon")) {
      throw new Error("Interact startup lease was replaced before state publication.");
    }
    writeState(paths, state());
