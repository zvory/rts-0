  ...
  async open() {
    ...
    this.child = this.spawnServer(binary, [], {
      ...
      env: {
        ...
        RTS_INTERACT_LAB_ARTIFACT_CAPABILITY: this.artifactCapability,
      },
      shell: false,
...
export function privateLoopbackUrl(value: string|URL) {
  ...
  if (!new Set(["127.0.0.1", "::1", "localhost"]).has(url.hostname) || !["http:", "https:"].includes(url.protocol)) {
    throw new PrivateServerError("invalidServerUrl", "Interact may reuse only a private loopback server.");
  }
...
function abortedError() {
  return new PrivateServerError("sessionClosed", "Interact driver was closed during server startup.");
}
