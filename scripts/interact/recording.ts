// Bounded real-time media recording for one Interact page.
...
export class InteractRecordingError extends Error {
  details: JsonObject;
  code: string;
  constructor(code: string, message: string, details: JsonObject = {}) {
    super(message);
    this.name = "InteractRecordingError";
    this.code = code;
...
export async function checkMediaCapabilities({
  ffmpeg = process.env.RTS_INTERACT_FFMPEG || "ffmpeg",
  ffprobe = process.env.RTS_INTERACT_FFPROBE || "ffprobe",
  requireH264 = true,
  ...
  try {
    ...
    throw new InteractRecordingError("ffmpegUnavailable", `FFmpeg was not available at ${JSON.stringify(ffmpeg)}: ${conciseProcessError(error)}`);
  }
  if (encoderCheck.status !== 0) throw new InteractRecordingError("ffmpegUnavailable", conciseToolFailure("FFmpeg capability check failed", encoderCheck));
  if (requireH264 && !/\blibx264\b/.test(encoderCheck.stdout || "")) throw new InteractRecordingError("h264Unavailable", "FFmpeg does not provide the libx264 encoder required for mobile-compatible MP4 output.");
  let probeCheck;
  try {
    probeCheck = await processRunner.run(ffprobe, ["-version"], { timeoutMs: 5_000, signal });
  } catch (error) {
    throw new InteractRecordingError("ffprobeUnavailable", `ffprobe was not available at ${JSON.stringify(ffprobe)}: ${conciseProcessError(error)}`);
  }
  if (probeCheck.status !== 0) throw new InteractRecordingError("ffprobeUnavailable", conciseToolFailure("ffprobe capability check failed", probeCheck));
  return {
    ffmpeg,
    ffprobe,
...
export async function createWallClockRecorder({ page, outputPath, clip, scale, tools, maxDurationMs, timeoutMs = 15_000 }: {
  ...
  const capturePhysicalFrame = () => {
    if (capturePromise || stopping) return;
    capturePromise = (async () => {
      const buffer = Buffer.from(await page.screenshot({ type: "png", captureBeyondViewport: false }) || []);
  ...
  try {
    ...
    const timer = setTimeout(() => firstFrameReject(new InteractRecordingError("recordingEmpty", "Chrome did not provide an initial screencast frame.")), timeoutMs);
    timer.unref?.();
  ...
  return {
    ...
    async stop() {
      ...
      try {
        ...
        if (extra > 0) {
          throw new InteractRecordingError("recordingClockDrift", `Recorder queued ${extra} frame slots beyond its measured wall duration.`);
        }
...
export async function finalizeMp4Artifacts({
  ...
  if (!stat || stat.size === 0) throw new InteractRecordingError("recordingEmpty", "FFmpeg produced no MP4 recording bytes.");
  if (stat.size > RECORDING_LIMITS.maxBytes) {
    fs.rmSync(mp4Path, { force: true });
    throw new InteractRecordingError("recordingTooLarge", `Final MP4 exceeded the ${RECORDING_LIMITS.maxBytes} byte limit and was deleted.`);
  }
  ...
  if (probe.container !== "mov,mp4,m4a,3gp,3g2,mj2" || probe.pixelFormat !== "yuv420p" || probe.codecTag !== "avc1" || !fastStart) {
    throw new InteractRecordingError("mediaProbeFailed", "Final MP4 is missing its mobile-compatible container, yuv420p/avc1 video, or fast-start metadata.");
  }
  probe.fastStart = fastStart;
  const expected = Math.max(1, Math.ceil(targetDurationMs * RECORDING_LIMITS.fps / 1000));
  if (probe.frameCount !== expected) throw new InteractRecordingError("mediaProbeFailed", `Final MP4 contains ${probe.frameCount ?? "an unknown number of"} frames; expected ${expected} from the wall clock.`);
  ...
  if (framePaths.length === 0) throw new InteractRecordingError("frameExtractionFailed", "FFmpeg produced no representative PNG frames.");
  await runTool(tools.ffmpeg, [
...
async function probeMedia(file: string, ffprobe: string, expectedCodec: string, label: string, processRunner: ProcessRunner, signal?: AbortSignal): Promise<ProbeResult> {
  ...
  try {
    ...
    throw new InteractRecordingError("mediaProbeFailed", `ffprobe could not inspect the ${label}: ${conciseProcessError(error)}`);
  }
  if (result.status !== 0) throw new InteractRecordingError("mediaProbeFailed", conciseToolFailure(`ffprobe rejected the ${label}`, result));
  let parsed: unknown;
  try { parsed = JSON.parse(result.stdout) as unknown; } catch { throw new InteractRecordingError("mediaProbeFailed", "ffprobe returned invalid JSON."); }
  if (!isJsonObject(parsed)) throw new InteractRecordingError("mediaProbeFailed", "ffprobe returned an invalid payload.");
  const streams = Array.isArray(parsed.streams) ? parsed.streams : [];
  const stream = isJsonObject(streams[0]) ? streams[0] : {};
  const format = isJsonObject(parsed.format) ? parsed.format : {};
  if (stream.codec_name !== expectedCodec) throw new InteractRecordingError("mediaProbeFailed", `Expected ${expectedCodec} ${label}, received ${stream.codec_name || "an unknown codec"}.`);
  return {
...
async function runTool(command: string, args: string[], label: string, timeoutMs: number = RECORDING_LIMITS.mediaStageTimeoutMs, processRunner = new ProcessRunner(), signal?: AbortSignal) {
  ...
  try {
    ...
    throw new InteractRecordingError("mediaProcessingFailed", `${label} failed: ${conciseProcessError(error)}`);
  }
  if (result.status !== 0) throw new InteractRecordingError("mediaProcessingFailed", conciseToolFailure(`${label} failed`, result));
}
  ...
  try {
    ...
    const [code, signal] = await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timer = setTimeout(() => {
          child.kill("SIGKILL");
          reject(new InteractRecordingError("recordingFinalizeTimeout", `Recording encoder did not finalize within ${timeoutMs}ms.`));
        }, timeoutMs);
        timer.unref?.();
      }),
    ]);
    if (spawnFailure()) throw encoderSpawnError(spawnFailure());
    if (code !== 0) throw new InteractRecordingError("mediaProcessingFailed", `H.264 encoder failed${signal ? ` (${signal})` : ""}: ${String(stderr() || "unknown failure").trim().slice(-800)}`);
  } finally {
    clearTimeout(timer);
  }
}
...
function encoderSpawnError(error: unknown) {
  return new InteractRecordingError(
...
function readPngDimensions(buffer: Buffer) {
  if (!Buffer.isBuffer(buffer) || buffer.length < 24 || buffer.toString("ascii", 1, 4) !== "PNG") throw new InteractRecordingError("contactSheetInvalid", "Contact sheet is not a valid PNG.");
