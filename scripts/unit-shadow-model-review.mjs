...
function validateCandidates() {
  if (PROJECTED_UNIT_SHADOW_MODEL_CANDIDATES.length !== 8) {
    throw new Error("The review sheet must contain all eight remaining non-plane unit kinds.");
  }
  const kinds = new Set();
  for (const entry of PROJECTED_UNIT_SHADOW_MODEL_CANDIDATES) {
    if (entry.kind === "scout_plane") throw new Error("Scout Plane is deliberately excluded.");
...
function renderDocument(view) {
  ...
  <html><head><meta charset="utf-8"><style>
    * { box-sizing: border-box; }
    html, body { width: 2400px; height: 900px; margin: 0; overflow: hidden; }
    ...
    main { display:grid; grid-template-columns: repeat(8, 300px); grid-template-rows: repeat(2, 406px); }
    article { position:relative; height:406px; border-right:1px solid #a39f8e; border-bottom:1px solid #a39f8e; background:rgba(248,246,237,.66); overflow:hidden; }
    ...
    <header><div><h1>Remaining unit shadow-model candidates · ${viewLabel}</h1>
      <div class="subtitle">Two candidates per unit · Scout Plane excluded · forward axis points right</div></div>
...
function boxCorners(part) {
  ...
  for (const dx of [-lx / 2, lx / 2]) for (const dy of [-ly / 2, ly / 2]) for (const dz of [-lz / 2, lz / 2]) {
    ...
    result.push([cx + dx * cos - dy * sin, cy + dx * sin + dy * cos, cz + dz]);
  }
  return result;
}
...
function boxFaces(part) {
  ...
  return indices.map(([face, normal]) => ({
    ...
    normal: [normal[0] * cos - normal[1] * sin, normal[0] * sin + normal[1] * cos, normal[2]],
  }));
}
