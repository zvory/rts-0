...
import { LAB_INTERACT_COMMANDS, requestTimeoutMs } from "./command_registry.ts";
    ...
    while (Date.now() < deadline) {
      ...
      if (startupError?.nonce === startupNonce || childFailure) {
        ...
        throw cliError(
          ...
          typeof startupError?.message === "string" ? startupError.message : errorMessage(childFailure || "Lab Interact daemon exited before readiness."),
        );
      }
      const state = readState(paths);
...
async function confirmStopped(paths: RuntimePaths, requestError: unknown): Promise<void> {
  ...
  try {
    ...
    if (hasErrorCode(error, "EEXIST")) throw requestError || cliError("daemonStarting", "Lab Interact startup is still in progress.");
    throw error;
  }
  try {
    const owner = readState(paths);
