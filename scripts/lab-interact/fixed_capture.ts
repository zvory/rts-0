...
export async function createFixedCaptureEncoder({
  ...
  return {
    ...
    async finish() {
      ...
      if (probeResult.status !== 0) throw new LabInteractRecordingError("mediaProbeFailed", `fixed capture probe failed: ${String(probeResult.stderr || "unknown failure").slice(-800)}`);
...
async function run(command: string, args: string[], label: string, processRunner: ProcessRunner, signal?: AbortSignal) {
  ...
  if (result.status !== 0) throw new LabInteractRecordingError("mediaProcessingFailed", `${label} failed: ${String(result.stderr || "unknown failure").slice(-800)}`);
}
