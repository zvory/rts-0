...
The CI Rust/architecture job installs `cargo-nextest` and invokes `tests/run-all.sh --only-rust`,
so the required Rust path is the same nextest-backed runner developers use locally.
