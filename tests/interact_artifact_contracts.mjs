...
import { INTERACT_LIMITS, InteractService, validateCommandInput } from "../scripts/interact/command_service.ts";
...
try {
  ...
  assert.ok(fs.realpathSync(setup.path).startsWith(fs.realpathSync(path.join(root, "target", "interact", "lab"))));
  ...
  assert.equal(INTERACT_LIMITS.maxAliases, 400, "portable artifacts share the operational alias bound");
  assert.ok(Buffer.byteLength(maximumSidecar) < INTERACT_LIMITS.maxAliasSidecarBytes, "64 KiB safely admits 400 maximum-length aliases and u32 ids");
  ...
  console.log("interact lab artifact contracts passed");
} finally {
