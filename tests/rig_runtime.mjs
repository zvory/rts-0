...
test("mortar PNG atlas covers unit route with tinted carriage, tinted tube, and fixed tires", () => {
  ...
  const packedTireSprite = MORTAR_TEAM_PNG_RIG_ATLAS.sprites.find((sprite) => sprite.id === "sprite.mortar.tire.top.packed");
  assert.ok(packedCarriageSprite);
  assert.ok(packedTubeSprite);
  assert.ok(packedTireSprite);
  assert.equal(packedCarriageSprite.tintSlot, "team-light");
  assert.equal(packedTubeSprite.tintSlot, "team-light");
  assert.equal(packedTireSprite.tintSlot, "fixed");
  ...
  assert.ok(packedTireSprite.sourceParts.includes("part.mortar.wheel.left.body.packed"));
  ...
  const topTire = unit.parts.get("sprite.mortar.tire.top.deployed").display;
  assert.equal(carriage.visible, true);
  assert.equal(tube.visible, true);
  assert.equal(topTire.visible, true);
  assert.equal(carriage.alpha, 1);
  assert.equal(tube.alpha, 1);
  assert.equal(topTire.alpha, 1);
  assert.notEqual(carriage.tint, 0xffffff);
  assert.notEqual(tube.tint, 0xffffff);
  assert.equal(topTire.tint, 0xffffff);
  ...
  const packedTopTire = unit.parts.get("sprite.mortar.tire.top.packed").display;
  assert.equal(packedCarriage.visible, true);
  assert.equal(packedTopTire.visible, true);
  assert.equal(packedCarriage.alpha, 1);
  assert.equal(packedTopTire.alpha, 1);
  assert.equal(carriage.alpha, 0);
  assert.equal(tube.alpha, 0);
  assert.equal(topTire.alpha, 0);
});
