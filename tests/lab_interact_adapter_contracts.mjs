...
console.log("✅ lab_interact_adapter_contracts.mjs: bounded processes, abort/reaping, and responsive lifecycle lanes passed");

async function processRunnerContracts() {
  const runner = new ProcessRunner({ maxOutputBytes: 128, termGraceMs: 30 });
  const shellMarker = path.join(os.tmpdir(), `lab-interact-shell-${process.pid}-${Date.now()}`);
...
async function responsiveServiceContracts() {
  ...
  const mediaService = new LabInteractService({ workspaceRoot: process.cwd(), driverFactory: async () => driver });
