...
import { InteractDriverError } from "../scripts/interact/driver.ts";
...
try {
  service = new InteractService({
    workspaceRoot: root,
    driverFactory: async (options) => {
      const driver = await openInteractDriver(options);
  ...
  assert.deepEqual(
    ...
    { count: 100, detailed: INTERACT_LIMITS.maxResponseDetails, truncated: true },
  ...
  assert.deepEqual(
    ...
    spawns.slice(0, INTERACT_LIMITS.maxResponseDetails).map((entry) => entry.alias),
  ...
  assert.deepEqual(
    ...
    { count: 400, detailed: INTERACT_LIMITS.maxResponseDetails, truncated: true },
  ...
  assert.ok(Math.max(...calls.inspectSizes) <= INTERACT_LIMITS.maxInspectRefs,
    "bulk reference resolution respects the bridge inspect bound");

  assert.equal(INTERACT_LIMITS.maxMutationBatch, 400, "mutation batch authority is 400");
  assert.equal(INTERACT_LIMITS.maxAliases, 400, "large scenes may retain 400 operational aliases");
  assert.equal(INTERACT_LIMITS.maxInspectResults, 400, "large-scene inspection may return 400 entities");
  ...
  const normalized = normalizeError(new InteractDriverError("labRejected", "blocked", {
    failedIndex: 3,
    attempted: { x: 10, y: 20 },
  ...
  console.log("✅ interact_bulk_contracts.mjs: bulk command service contracts passed");
} finally {
