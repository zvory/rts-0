...
const CATALOG = Object.freeze({
  ...
  supportedCommandKinds: [
    ...
    "adjustProductionRepeat", "research", "cancel", "stop", "holdPosition", "setRally",
  ],
...
export async function openInteractDriver(options) {
  ...
  return {
    ...
    async spawn(spawns) {
      const delayMs = Number(process.env.RTS_INTERACT_FAKE_DELAY_MS || 0);
    ...
    async captureFixed({ sessionId, name = "fixed", fps = 30, frameCount = 30, sceneIdentity = null, sceneRevision = 0, aliases = [] }) {
      ...
      const directory = `${options.workspaceRoot}/target/interact/lab/${sessionId}/fixed/${name}`;
