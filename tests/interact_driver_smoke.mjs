// Private-server Interact smoke. Run directly; it owns and cleans up its browser and server.
import assert from "node:assert/strict";
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";

import { InteractDriver } from "../scripts/interact/driver.ts";

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
let driver;
try {
  driver = await InteractDriver.open({
    workspaceRoot: root,
  ...
  const capture = await driver.screenshot({
    ...
    request: { tool: "interact_driver_smoke" },
  });
  ...
  const twoEntityCapture = await driver.screenshot({
    ...
    request: { tool: "interact_driver_smoke", fixture: "two-entity" },
  });
  ...
  assert.deepEqual(diagnostics.pageConsoleErrors, [], "Interact page has no console errors");
  assert.deepEqual(diagnostics.pageErrors, [], "Interact page has no frame errors");
  await driver.remove([firstId, secondId, correctedId]);
  console.log("✅ interact_driver_smoke.mjs: private server, bridge, mutation, order, time, camera, two captures, and cleanup passed");
} finally {
  await driver?.close();
}
