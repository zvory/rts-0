...
(async () => {
  ...
  ok(snap.minerals === 50, `A starts with 50 minerals (${snap.minerals})`);
  ok(snap.gas === 0, `A starts with 0 gas (${snap.gas})`);
  ...
  ok(mine.filter((e) => e.kind === "hq").length === 1, `A owns 1 HQ`);
  const workers = mine.filter((e) => e.kind === "worker");
  ok(workers.length === 4, `A owns 4 workers (${workers.length})`);
  const minerals = snap.entities.filter((e) => e.kind === "minerals");
  ok(minerals.length > 0 && minerals[0].owner === 0, `A sees neutral mineral nodes (${minerals.length})`);
  ok(!snap.entities.some((e) => e.owner === B.playerId), `FOG: A cannot see B at start`);

  A.send({ t: "command", cmd: { c: "gather", units: workers.map((w) => w.id), node: minerals[0].id } });
  let sawCarry = false, peak = snap.minerals;
  for (let i = 0; i < 30; i++) {
    await sleep(500);
    if (A.lastSnapshot) {
      peak = Math.max(peak, A.lastSnapshot.minerals);
      if (A.lastSnapshot.entities.some((e) => e.owner === A.playerId && e.kind === "worker" && (e.carrying || 0) > 0)) sawCarry = true;
      if (A.lastSnapshot.minerals > 50) break;
    }
  }
  ok(peak > 50, `GATHER: minerals rose above 50 (peak=${peak})`);
  ok(sawCarry, `GATHER: a worker carried minerals`);

  const beforeTrain = A.lastSnapshot.minerals;
  A.send({ t: "command", cmd: { c: "train", building: mine.find((e) => e.kind === "hq").id, unit: "worker" } });
  await sleep(1200);
  ...
  ok(A.lastSnapshot.minerals <= beforeTrain - 50 + trainIncomeMargin, `TRAIN: minerals dropped ~50 (before=${beforeTrain}, after=${A.lastSnapshot.minerals})`);
  const hq = A.lastSnapshot.entities.find((e) => e.kind === "hq" && e.owner === A.playerId);
  ok(hq && (hq.prodKind === "worker" || (hq.prodQueue || 0) >= 1), `TRAIN: HQ shows production (queue=${hq?.prodQueue})`);
