...
const artifactRoot = path.join(workspaceRoot, "target", "lab-interact", "lab_test", "captures");
...
try {
  assert.equal(LAB_INTERACT_PREVIEW_TTL_MS, 24 * 60 * 60 * 1000, "Lab artifacts receive a full 24-hour retention window");
  ...
  assert.ok(published.expiresAt >= Date.now() + LAB_INTERACT_PREVIEW_TTL_MS - 1_000, "preview expiry is at least 24 hours after publication");
...
const validationArtifacts = path.join(validationRoot, "target", "lab-interact", "lab_test", "captures");
...
const validationPreview = new LabInteractTailnetPreview({
  workspaceRoot: validationRoot,
  publishArtifact: async () => ({ url: "http://127.0.0.1:8091/p/fixture/scene.png", expiresAt: Date.now() + LAB_INTERACT_PREVIEW_TTL_MS }),
});
try {
  assert.throws(
    () => new LabInteractTailnetPreview({ workspaceRoot: validationRoot, ttlMs: LAB_INTERACT_PREVIEW_TTL_MS - 1 }),
...
console.log("✅ lab_interact_tailnet_preview_contracts.mjs: durable 24-hour Tailnet artifact delivery passed");
