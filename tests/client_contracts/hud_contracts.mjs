...

{
  ...
  assert(buildCard.slots[1].label === "Pump Jack" && buildCard.slots[1].hotkey === "W" &&
    buildCard.slots[1].cost.steel === 150, "Engineer build menu restores the 150-Steel Pump Jack in the W slot");
  ...
  assert(
    pumpJackTooltip.includes("36s") && pumpJackTooltip.includes("10s") && pumpJackTooltip.includes("150 Steel") &&
  ...
  const portalCard = buildCommandCardDescriptors(commandCardCtx({
    selection: [portal],
    entities: [portal],
    factionId: "cultivators",
    resources: { steel: 100, oil: 0 },
  }));
  assert(
    portalCard.slots[0].label === "Warrior" && portalCard.slots[0].hotkey === "Q" && portalCard.slots[0].enabled,
    "Cultivator Portal trains an affordable Warrior from hotkey Q",
  );
  assert(
    portalCard.slots[0].cost.steel === 100 && STATS[KIND.WARRIOR].supply === 2,
    "Warrior train descriptor exposes its 100-Steel and 2-Supply cost",
  );
