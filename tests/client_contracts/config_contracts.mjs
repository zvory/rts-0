...
// ---------------------------------------------------------------------------
{
  ...
  assert(
    STATS[KIND.RESOURCE_DEPOT].cost.steel === 450 &&
      ...
      STATS[KIND.RESOURCE_DEPOT].buildTicks === 150,
  ...
  assert(
    STATS[KIND.BARRACKS].trains.length === 3 &&
      ...
      trainableUnitsForFaction("kriegsia", KIND.BARRACKS).includes(KIND.PANZERFAUST),
  ...
  assert(
    !commandCardAbilitiesForFaction().some((entry) => entry.ability === ABILITY.BLANKET_FIRE),
