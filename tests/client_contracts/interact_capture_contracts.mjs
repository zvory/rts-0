...
Renderer.prototype._beginRenderFrame.call(renderer);
readiness = Renderer.prototype.captureReadiness.call(renderer, { subjectIds: [9], subjectKinds: ["tank"] });
assert.deepEqual(readiness.missingTextureSubjectIds, [], "a later rendered frame clears an earlier transient texture fallback");
renderer._renderErrors.set("unit:tank", { count: 1, lastMessage: "transient draw failure" });
renderer._lastRenderErrorFrame = renderer._renderFrameCount - 1;
readiness = Renderer.prototype.captureReadiness.call(renderer, { subjectIds: [], subjectKinds: ["tank"] });
assert.deepEqual(readiness.renderErrors, [], "a later rendered frame clears an earlier transient draw failure");
renderer._lastRenderErrorFrame = renderer._renderFrameCount;
readiness = Renderer.prototype.captureReadiness.call(renderer, { subjectIds: [], subjectKinds: ["tank"] });
assert.equal(readiness.renderErrors.length, 1, "a draw failure in the current frame blocks capture readiness");
renderer._renderErrors.clear();
renderer._lastRenderErrorFrame = -1;
...
const captureBridge = new InteractBridge({
  enabled: true,
  windowLike: {},
  app: {
    net: { ws: { readyState: 1 } },
...
assert.deepEqual(calls, ["presentation:true"], "presentation delegates its one resize to the app shell rather than duplicating it");
