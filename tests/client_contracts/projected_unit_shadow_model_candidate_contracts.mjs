...
for (const unit of PROJECTED_UNIT_SHADOW_MODEL_CANDIDATES) {
  ...
  for (const candidate of unit.candidates) {
    ...
    for (const part of candidate.parts) {
      ...
      assert(Number.isFinite(part.pitch));
    }
...
for (const excluded of ["scout_plane", "ekat", "worker", "golem"]) {
  assert(!PROJECTED_UNIT_SHADOW_MODEL_CANDIDATES.some((entry) => entry.kind === excluded));
}
for (const raised of ["mortar_team", "artillery"]) {
  const unit = PROJECTED_UNIT_SHADOW_MODEL_CANDIDATES.find((entry) => entry.kind === raised);
  assert(unit.candidates.every((candidate) => candidate.parts.some((part) => part.id === "barrel" || part.id === "tube")));
  assert(unit.candidates.every((candidate) => candidate.parts.some((part) => part.pitch > 0.3)));
}
