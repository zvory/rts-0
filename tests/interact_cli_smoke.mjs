...
import { processAlive, runtimePaths, sleep } from "../scripts/interact/runtime.ts";
...
console.log("✅ interact_cli_smoke.mjs: semantic scene, setup round trip, PNG preview, H.264 recording, and teardown passed");

function invoke(command, input = {}) {
  return spawnSync(process.execPath, [cli, "lab", command, JSON.stringify(input)], {
    cwd: root,
    env,
    encoding: "utf8",
...
function invokeAsync(command, input = {}) {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [cli, "lab", command, JSON.stringify(input)], {
      cwd: root,
      env,
...
function labArtifactPath(activeSessionId, category, suffix, directoryPrefix = "") {
  const directory = path.join(root, "target", "interact", "lab", activeSessionId, category);
