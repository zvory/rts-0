...
#[test]
fn supply_stress_dev_scenario_launches_an_active_prediction_recipient() {
    let mut task = RoomTask::new(
        "active-supply-stress-start".to_string(),
        RoomMode::DevScenario(DevScenarioConfig {
            id: DevScenarioId::SupplyStressActive,
    ...
    task.on_join(99, "Measured player".to_string(), false, false, msg_tx, ack);
    ...
    assert!(payload.prediction_build_id.is_some());
    assert!(payload.prediction_version > 0);
    assert!(!task.players.get(&99).unwrap().spectator);
}

#[test]
fn supply_stress_active_recipient_gets_player_projection_and_prediction_acks() {
    let mut task = RoomTask::new(
        "active-supply-stress-snapshot".to_string(),
        RoomMode::DevScenario(DevScenarioConfig {
            id: DevScenarioId::SupplyStressActive,
            unit: EntityKind::Worker,
            count: 200,
    ...
    let snapshot = writer.snapshots.take().expect("active dev snapshot");
    assert_eq!(snapshot.entities.len(), 135);
    assert!(snapshot.net_status.prediction_version > 0);
    assert!(snapshot
        .entities
        .iter()
        .any(|entity| entity.owner == 1 && entity.kind == "city_centre"));
    assert!(!snapshot
        .entities
        .iter()
        .any(|entity| entity.owner == 2 && entity.kind == "city_centre"));
}
