...
//! private Lab Interact server receives an unguessable capability from its owning driver and all
...
pub(super) const CAPABILITY_ENV: &str = "RTS_LAB_INTERACT_ARTIFACT_CAPABILITY";
pub(super) const CAPABILITY_HEADER: &str = "x-lab-interact-capability";
const ROOM_HEADER: &str = "x-lab-interact-room";
...
pub(super) struct LabInteractArtifactBridge {
...
impl LabInteractArtifactBridge {
...
pub(super) async fn cleanup_handler(
    ...
    let removed = state.lab_interact_artifacts.remove_for_room(&request.room);
...
mod tests {
    ...
    fn bridge_is_disabled_without_capability() {
        let bridge = LabInteractArtifactBridge::default();
    ...
    fn transfer_ids_and_room_names_are_strict() {
        assert!(valid_room("labinteract-deadbeef-12"));
    ...
    fn capability_peer_and_expiry_are_enforced() {
        let bridge = LabInteractArtifactBridge {
