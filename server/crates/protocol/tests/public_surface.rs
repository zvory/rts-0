use rts_protocol::{
    ...
    GAME_CHECKPOINT_CURRENT_VERSION, LAB_REPLAY_ARTIFACT_KIND, LAB_REPLAY_ARTIFACT_SCHEMA,
...
fn stable_rust_public_surface_compiles() {
    ...
    assert_eq!(terrain::GRAVEL_A, 8);
    assert_eq!(terrain::GRAVEL_B, 9);
    assert_eq!(terrain::GRAVEL_C, 10);
    assert_eq!(terrain::DIRT_A, 11);
    assert_eq!(terrain::DIRT_B, 12);
    assert_eq!(terrain::DIRT_C, 13);
    assert_eq!(terrain::MUD_A, 14);
    assert_eq!(terrain::MUD_B, 15);
    assert_eq!(terrain::MUD_C, 16);
    assert_eq!(terrain::FROSTED_GROUND, 17);
    assert_eq!(terrain::ALL, (0_u8..=17).collect::<Vec<_>>().as_slice());
    assert!(terrain::ALL.iter().copied().all(terrain::is_known));
    ...
    assert_eq!(COMPACT_SNAPSHOT_VERSION, 55);
