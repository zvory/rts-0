...
#[test]
fn ekaterina_mark_target_spends_cools_down_and_damages_units_after_delay() {
    let players = [
        PlayerInit {
            id: 1,
            team_id: 1,
    ...
    let mut game = empty_flat_game(&players);
    game.players[0].refund_resources(15, 25);
    let target = game.map.tile_center(12, 12);
    let caster_pos = game.map.tile_center(5, 12);
    let caster = game
        .entities
        .spawn_unit(
            1,
        ...
        .expect("signal team should spawn");
    let enemy = game
        .entities
        .spawn_unit(2, EntityKind::CommandCar, target.0 + 8.0, target.1)
        .expect("enemy should spawn");
    let friendly = game
        .entities
        .spawn_unit(1, EntityKind::CommandCar, target.0 - 8.0, target.1)
    ...
    game.enqueue(
        1,
        Command::UseAbility {
            ability: ability::AbilityKind::MarkTarget,
            units: vec![caster],
            x: Some(target.0),
            y: Some(target.1),
            queued: false,
    ...
    let owner_events = launch_events
        .iter()
        .find(|(pid, _)| *pid == 1)
        .map(|(_, events)| events.as_slice())
        .unwrap_or(&[]);

    assert_eq!(game.players[0].steel, 85);
    assert!(
        game.entities
            .get(caster)
            .expect("caster should survive")
            .ability_cooldown_ticks(ability::AbilityKind::MarkTarget)
            > 0
    );
    assert!(owner_events.iter().any(|event| matches!(
        ...
        Event::MarkTarget {
            ...
            && (*x - target.0).abs() < 0.1
            && (*y - target.1).abs() < 0.1
            && (*radius_tiles - config::MARK_TARGET_RADIUS_TILES).abs() < 0.01
            && *delay_ticks == config::MARK_TARGET_DELAY_TICKS
    )));
    assert_eq!(game.entities.get(enemy).expect("enemy").hp, 225);
    assert_eq!(game.entities.get(friendly).expect("friendly").hp, 225);

    let mut impact_events = Vec::new();
    for _ in 0..config::MARK_TARGET_DELAY_TICKS {
        impact_events = game.tick();
    }

    assert_eq!(game.entities.get(enemy).expect("enemy").hp, 205);
    assert_eq!(
        game.entities.get(friendly).expect("friendly").hp,
        205,
        "Mark Target has the approved friendly-fire support-weapon contract"
    );
    assert!(impact_events.iter().any(|(pid, events)| {
        *pid == 1
            && events.iter().any(|event| {
                matches!(
                    event,
                    Event::MarkTargetImpact { x, y, radius_tiles }
                        if (*x - target.0).abs() < 0.1
                            && (*y - target.1).abs() < 0.1
                            && (*radius_tiles - config::MARK_TARGET_RADIUS_TILES).abs() < 0.01
                )
            })
...
#[test]
fn mark_target_is_rejected_for_wrong_faction_carrier() {
    let players = [PlayerInit {
        id: 1,
        team_id: 1,
        faction_id: crate::rules::faction::DEFAULT_FACTION_ID.to_string(),
    ...
    let mut game = empty_flat_game(&players);
    let starting_steel = game.players[0].steel;
    let target = game.map.tile_center(12, 12);
    let caster = game
        .entities
        .spawn_unit(1, EntityKind::EkaterinaSignalTeam, target.0, target.1)
        .expect("signal team should spawn");
    game.spatial = services::spatial::SpatialIndex::build(&game.entities, game.map.size);
    ...
    assert_eq!(game.players[0].steel, starting_steel);
    assert_eq!(
        game.entities
            .get(caster)
            .expect("caster should survive")
    ...
    assert!(events
        ...
        .all(|event| !matches!(
            event,
            Event::MarkTarget { .. } | Event::MarkTargetImpact { .. }
        )));
}

#[test]
fn mark_target_marker_does_not_reveal_hidden_caster_to_enemy() {
    let players = [
        PlayerInit {
            id: 1,
            team_id: 1,
    ...
    let mut game = empty_flat_game(&players);
    let caster_pos = game.map.tile_center(4, 12);
    let target = game.map.tile_center(12, 12);
    let enemy_spotter_pos = game.map.tile_center(19, 12);
    let caster = game
        .entities
        .spawn_unit(
            1,
    ...
    assert!(enemy_events.iter().any(|event| matches!(
        event,
        Event::MarkTarget { from: None, x, y, .. }
            if (*x - target.0).abs() < 0.1 && (*y - target.1).abs() < 0.1
    )));
    assert!(enemy_events.iter().all(|event| {
        !matches!(
            event,
            Event::MarkTarget { from: Some(_), .. } | Event::Attack { .. }
        )
    }));
}
