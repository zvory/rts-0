...
#[test]
fn tank_trap_zero_sight_does_not_reveal_owner_fog() {
    let players = [
        PlayerInit {
            id: 1,
            team_id: 1,
    ...
    assert!(
        !game.fog.is_visible_world(1, x, y),
        "zero-sight Tank Traps must not reveal their own tile"
    );
}
...
fn tank_trap_can_be_damaged_and_removed_by_death_cleanup() {
    ...
    let mut events: HashMap<u32, Vec<Event>> =
        game.players.iter().map(|player| (player.id, Vec::new())).collect();
    services::death::death_system(
        &mut game.entities,
        &game.fog,
        &game.smokes,
