...
#[allow(clippy::type_complexity)]
pub(super) fn tank_trap_line_build_map(
    ...
    let training_pos =
        services::occupancy::footprint_center(&map, EntityKind::TrainingCentre, center.0 - 8, center.1 + 6);
    ...
    let (unit_starts, goal) = match layout {
        TankTrapLineLayout::Horizontal => {
            ...
            let goal = ((center.0 as f32 + 5.0) * ts, y);
            (vec![(start_x, y - vehicle_gap), (start_x, y + vehicle_gap)], goal)
        }
        TankTrapLineLayout::Vertical => {
            let x = (center.0 as f32 - 1.5) * ts;
            let start_y = (center.1 as f32 - 7.0) * ts;
            let goal = (x, (center.1 as f32 + 3.0) * ts);
            (vec![(x - vehicle_gap, start_y), (x + vehicle_gap, start_y)], goal)
        }
        TankTrapLineLayout::Diagonal => {
            let start = ((center.0 as f32 - 6.0) * ts, (center.1 as f32 - 6.0) * ts);
            let goal = ((center.0 as f32 + 4.0) * ts, (center.1 as f32 + 4.0) * ts);
            (vec![start, (start.0, start.1 + vehicle_gap * 2.0)], goal)
        }
    };
    if let Some(slot) = map.starts.get_mut(0) {
        *slot = center;
...
pub(super) enum TankTrapLineLayout {
    Horizontal,
    Vertical,
    Diagonal,
}
...
pub(super) fn spawn_tank_trap_line_test_units(
    entities: &mut EntityStore,
    vehicle: EntityKind,
    starts: Vec<(f32, f32)>,
) -> Result<Vec<u32>, String> {
...
pub(super) fn spawn_tank_trap_line_workers(
    entities: &mut EntityStore,
    starts: Vec<(f32, f32)>,
) -> Result<(), String> {
    for (x, y) in starts {
...
fn vehicle_line_start_gap(unit: EntityKind) -> f32 {
    match unit {
        EntityKind::AntiTankGun => config::ANTI_TANK_GUN_BODY_LENGTH_PX,
        EntityKind::MortarTeam => config::ANTI_TANK_GUN_BODY_LENGTH_PX,
