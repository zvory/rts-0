...
const OPENING_SECONDS: u32 = 60;
const LIVE_WINDOW_SECONDS: u32 = 10;

impl Game {
    pub(super) fn current_apm(&self, player_id: u32) -> u32 {
        let window_ticks = config::TICK_HZ.saturating_mul(LIVE_WINDOW_SECONDS);
...
fn rolling(action_ticks: impl Iterator<Item = u32>, current_tick: u32) -> u32 {
    let window_ticks = config::TICK_HZ.saturating_mul(LIVE_WINDOW_SECONDS);
    ...
    actions
        .saturating_mul(60)
        .checked_div(LIVE_WINDOW_SECONDS as u64)
        .unwrap_or(0)
        .min(u32::MAX as u64) as u32
}

pub(super) fn average_after_opening(
    action_ticks: impl Iterator<Item = u32>,
    duration_ticks: u32,
) -> u32 {
    let opening_ticks = config::TICK_HZ.saturating_mul(OPENING_SECONDS);
...
mod tests {
    ...
    fn score_apm_excludes_first_minute_and_counts_each_command_once() {
        ...
        assert_eq!(average_after_opening(ticks.into_iter(), minute * 2), 3);
    }

    #[test]
    fn score_apm_is_zero_until_the_opening_minute_ends() {
        assert_eq!(average_after_opening([1, 30, 900].into_iter(), 1799), 0);
    }
    ...
    fn rolling_apm_counts_command_envelopes_in_the_last_ten_seconds() {
        let window = config::TICK_HZ * LIVE_WINDOW_SECONDS;
        assert_eq!(rolling([0].into_iter(), 1), 6);
        assert_eq!(rolling([0, 1, window].into_iter(), window), 18);
        assert_eq!(
            rolling([0, 1, window, window + 1].into_iter(), window + 1),
            12
        );
    }
}
