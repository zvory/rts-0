...
#[test]
fn direct_fire_legality_allows_anti_tank_gun_shots_at_infantry() {
    let map = open_map(12);
    let smokes = SmokeCloudStore::new();
    ...
    for target_kind in [
        ...
        for legality in [
            ...
            assert!(
                ...
                "{legality:?} should allow anti-tank gun fire at {target_kind}"
            );
        }
        assert!(
            ...
            "direct Attack commands should allow {target_kind}"
        );
    }
}
...
#[test]
fn direct_fire_legality_waits_for_sampled_smoke_melee_visibility() {
    let map = open_map(12);
    let mut entities = EntityStore::new();
    let attacker_pos = map.tile_center(4, 4);
    let target_pos = map.tile_center(5, 4);
    ...
    assert!(!direct_fire_target_legal(
        &map,
        &entities,
        &blockers,
        &teams,
    ...
    assert!(
        ...
        "an explicit attack must wait for the same authoritative visibility sample"
    );
}
