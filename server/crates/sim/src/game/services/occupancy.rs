...
impl OccupancyData {
    fn build(map: &Map, entities: &EntityStore) -> Self {
        ...
        let mut tree_path_cost = vec![false; (size * size) as usize];
        let mut tree_trunks_by_tile = vec![Vec::new(); (size * size) as usize];
        ...
        let all_ground_static_fingerprint =
            static_path_fingerprint(size, &all_ground_static_blocked, &tree_path_cost);
        let vehicle_body_static_fingerprint =
            static_path_fingerprint(size, &vehicle_body_static_blocked, &tree_path_cost);
...
impl Occupancy<'_> {
    ...
    pub(crate) fn tree_trunks_in_tile_rect(
        ...
        let max_tx = max_tx.min(size - 1);
        let max_ty = max_ty.min(size - 1);
...
fn static_path_fingerprint(size: u32, static_blocked: &[bool], tree_path_cost: &[bool]) -> u64 {
    let mut hash = static_blocked_fingerprint(size, static_blocked);
