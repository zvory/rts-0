...
use crate::game::services::pathing::{
    tree_detour_between, PathCacheStatus, PathRequest, PathingRequestDiagnostics,
...
impl<'a> MoveCoordinator<'a> {
    ...
    fn request_path(
        ...
        } = self.pathing.request_finalized_with_diagnostics(
            self.map,
            self.occ,
            req,
            (start_pos, goal),
...
mod tests {
    ...
    fn build_staging_goal_uses_outside_tile_for_worker_approaching_footprint() {
        ...
        let footprint: std::collections::BTreeSet<(u32, u32)> =
            footprint_tiles(EntityKind::ResourceDepot, 20, 20)
                .into_iter()
                .collect();
        assert!(
            !footprint.contains(&tile),
            "staging goal must be outside the 3x3 Resource Depot footprint, got {tile:?}"
        );
    }
    ...
    fn build_order_accepts_long_expansion_route_to_outside_staging() {
        ...
        let ok = coordinator.order_build(&mut entities, worker, EntityKind::ResourceDepot, 48, 70);

        assert!(
            ok,
            "expansion Resource Depot build order should find a staged route"
        );
        let e = entities.get(worker).unwrap();
        let goal = e.path_goal().expect("build order should set a path goal");
        let goal_tile = map.tile_of(goal.0, goal.1);
        let footprint: std::collections::BTreeSet<(u32, u32)> =
            footprint_tiles(EntityKind::ResourceDepot, 48, 70)
                .into_iter()
                .collect();
        assert!(
            !footprint.contains(&goal_tile),
            "build path goal should stop outside the expansion Resource Depot footprint"
        );
        assert!(
            build_staging_goal_in_range(&map, EntityKind::ResourceDepot, 48, 70, goal),
    ...
    fn smooth_vehicle_paths_use_clearance_route_shape() {
        ...
        for kind in [
            ...
            assert_eq!(
                ...
                Some(RoutePolicy::FastestTerrainTime)
            );
        }
    }
