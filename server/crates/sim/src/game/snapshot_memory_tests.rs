...
#[test]
fn fog_visibility_is_held_between_15_hz_samples() {
    ...
    game.state
        ...
        .set_position(hidden_pos.0, hidden_pos.1);
    game.tick();
    assert!(
        game.snapshot_for(1)
            .entities
            .iter()
            .all(|entity| entity.id != enemy),
        "the odd tick should keep the preceding fog sample"
    );
    ...
    assert!(
        game.snapshot_for(1)
            .entities
            .iter()
            .any(|entity| entity.id == enemy),
        "the even tick should publish the next fog sample"
    );
    ...
    assert!(
        game.snapshot_for(1)
            ...
            .iter()
            .any(|entity| entity.id == enemy),
        "the new sample should remain authoritative for the following odd tick"
    );
    ...
    assert!(
        game.snapshot_for(1)
            ...
            .iter()
            .all(|entity| entity.id != enemy),
        "the next even tick should replace the held sample"
    );
}
