...
end-to-end coverage. Public lobby semantics are already strict by this phase: empty public lobbies
are deleted immediately, with only the pending-create lease as an exception. Internal rooms should
either be deleted when empty or documented as intentionally retained for a non-public state-source
reason.
...
- Keep any intentionally retained internal room mode private, and document why it cannot be removed
  yet. Retained internal modes must not reserve public lobby names.
...
full or in-progress public room remains visible and correctly disabled while occupied, and that an
empty public lobby does not remain reserved for reconnect.

## Handoff

After this phase, report which room modes are disposable, which are intentionally retained, the
browser/integration tests that cover the policy, and any docs updated. Confirm that any retained
internal room mode is private and does not weaken the strict empty-public-lobby deletion rule. Call
out any remaining internal room mode that still needs a follow-up cleanup plan.
