...
- Add a capture clock that freezes and advances by exact milliseconds without globally patching
  ...
  control.
- Add a clearly named `lab-interact capture-fixed` command requiring the current opaque
  `sessionId` that:
...
- Lab Interact bridge/driver/daemon and CLI fixed-capture schemas
...
- Keep all generated paths confined beneath `target/lab-interact/` and clean transient sequences on
