...
Status: Initial inventory complete in Phase 0. Phase 3A must update this matrix while renaming the
temporary `steel_vanguard` id to the canonical Kriegsia id, `kriegsia`. Every later phase that
touches a match lifecycle path must keep this matrix updated.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Normal lobby start | `LobbyPlayer.factionId` and `PlayerInit.faction_id`, defaulted to `kriegsia` after Phase 3A rename | Kriegsia only | Kriegsia-only | Enabled for local Kriegsia player when build/version metadata is compatible | Schema 2 records player faction id | `tests/server_integration.mjs`, Phase 3A validation test | Selection UX remains hidden until rollout. |
| Quickstart/debug start | `PlayerInit.faction_id`, defaulted to `kriegsia` after Phase 3A rename; debug loadout remains separate | Kriegsia only; fixture only in explicit test/dev harnesses | Kriegsia-only | Disabled only for unsupported local-player faction | Schema 2 records faction id and current loadout shim | `server/crates/sim/src/game/setup/tests.rs`, Phase 3A/3D tests | No implicit Ekaterina or fixture quickstart. |
| AI add/remove/start | AI `PlayerInit.faction_id`, defaulted to `kriegsia` at match start | Kriegsia only | Kriegsia-only; unsupported faction requests reject or no-op before seat creation | Not applicable | Schema 2 records AI faction if match starts | `tests/server_integration.mjs`, Phase 3B validation test | `addAi` remains team-only in Phase 3. |
| Fixture/dev faction start | Explicit Rust test/dev harness only | `phase2_empty_fixture` test/dev-only; not normal lobby selectable | Reject AI unless explicitly allowed by a later phase | Disabled when local fixture player is unsupported | Record fixture id/loadout only in explicit test artifacts | `scripts/check-faction-assumptions.mjs`, Phase 3A/3D fixture tests | Fixture is architecture coverage, not a product faction. |
...
| Dev scenarios | Scenario definition plus Kriegsia default start unless scenario explicitly owns a test fixture | Kriegsia only in Phase 3; fixture only in explicit test/dev harnesses | Not applicable unless scenario declares AI | Disabled only for unsupported local-player faction | Not replayed unless scenario recording exists | `server/crates/sim/src/game/setup/dev_scenarios/tests.rs`, `docs/context/testing.md`, Phase 3D tests | No arbitrary client spawning. |
| Self-play | Self-play `PlayerInit.faction_id`, defaulted to `kriegsia` after Phase 3A rename | Kriegsia until AI support expands | Kriegsia-only | Not applicable | Artifact schema 2 records faction ids | `server/crates/ai/src/selfplay` tests, Phase 3D artifact tests | Separate AI plan needed for other factions. |
