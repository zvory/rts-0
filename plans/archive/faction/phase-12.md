# Phase 12 - Roster Expansion, Integration, and Rollout
...
- If lobby faction selection is exposed here, retire or clearly preserve any dev-only assignment
  path introduced by earlier phases.
...
- Reconcile every row in the faction lifecycle matrix and update it with final rollout status.
...
- Lifecycle matrix audit showing every creation/playback/spectator/dev/AI/prediction path is
  either supported, dev-gated, or explicitly rejected.
...
The handoff must state whether faction selection is ready for normal playtesting, whether dev-only
assignment paths remain, whether AI support is enabled or intentionally blocked, which docs/context
capsules were updated, lifecycle matrix status, remaining balance risks, and recommended playtest
scenarios.
