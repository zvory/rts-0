# Phase 1 - Reusable Interact Driver
...
Provide a transport-independent `InteractDriver` that opens and controls a small authoritative
...
- A narrow in-page `InteractBridge`, installed only behind the agent-tool launch gate, is composed
...
```js
const driver = await InteractDriver.open({ workspaceRoot, map, seed, scenario, viewport });
...
- the focused driver/helper area, renamed by Phase 0 under `interact`
