...
Replace the unusable project-scoped MCP integration with `interact`, a stateful command-line
...
- Deeply rename the agent tool from Agent Lab/agentlab/agent-lab to Interact/interact in
...
- Rename `AgentLabDriver` and `AgentLabBridge` to their `Interact` equivalents and update every
  caller. Preserve the driver/bridge authority and dependency-injection boundaries proven in
  Phase 1.
- Add a repository-owned `interact` CLI with discrete, scriptable commands for session
...
- Move ignored outputs to `<worktree>/target/interact/lab/` and update path confinement,
...
interact lab reset
interact lab close
interact lab shutdown
interact lab catalog [options]
interact lab spawn [options]
interact lab update [options]
interact lab remove [options]
interact lab order [options]
interact lab time [options]
interact lab inspect [options]
interact lab camera [options]
interact lab screenshot [options]
```
...
- `.gitignore`, `AGENTS.md`, `.agents/skills/interact/`, and deletion of the old skill path
...
- Do not delete source artifacts or unrelated processes during stale-state recovery. Cleanup must
  be restricted to resources proven to belong to the current worktree's Interact runtime.
...
- Add an end-to-end CLI smoke that starts from no daemon, opens, copies its `sessionId` into later
  ...
  captures a PNG under `target/interact/lab/`, resets, closes, verifies session children are gone,
...
- Create and capture a two-unit scene using only CLI commands, inspect the PNG once, and confirm its
  manifest identifies the selected worktree and new `target/interact/lab/` path.
