...
Phase 0 deletes it and maps the retained contract onto the `interact` CLI and daemon IPC.
...
```text
interact <command> [bounded options or validated JSON]
  -> private versioned local IPC
    -> one worktree daemon and alias table
      -> InteractDriver method
```
