...
Complete the initial Interact milestone with reliable visual capture. An agent can arrange a
...
- PNG and JSON manifests live under
  `<worktree>/target/interact/lab/<session-generation>/captures/` after the Phase 0 rename.
...
- The focused Interact skill and repository graphics guidance keep scenes small, invoke the
...
- `.agents/skills/interact/SKILL.md` after Phase 0 deletes the old skill path
...
- Current output paths, launch gates, module/class names, diagnostics, and skill names must use Lab
  Interact/interact after Phase 0.
