...
surface is the repository-owned `interact` CLI backed by one automatically started per-worktree
daemon, a reusable `InteractDriver`, a private Rust server, one headless normal client, stable
entity aliases, and bounded local artifacts. The existing human-facing Lab remains the product and
authority boundary: Interact composes typed Lab operations, normal commands, room time,
...
that adapter and configuration, deeply renames the agent tool to Interact, and migrates the
...
Codex or another local shell caller
  -> interact CLI (one bounded command, JSON result)
    -> private local IPC for the canonical Git worktree
      -> auto-started interact daemon (30-minute interaction-idle lifetime)
        -> InteractDriver (process/session/alias/artifact ownership)
        ...
        -> bounded files under target/interact/lab/
```
...
- Keep integration local. Use private local IPC plus loopback child services; do not add a
  production remote-control endpoint or make deployed rooms discoverable through Interact.
...
- Restrict default outputs to `<worktree>/target/interact/lab/`, use safe generated names, and
...
- Preserve the ordinary human-facing Lab vocabulary for the product and wire protocol. Deep rename
  ...
  artifacts from Agent Lab/agentlab/agent-lab to Interact/interact.
...
### [Phase 1 - Reusable Interact Driver](phase-1.md)
...
```bash
scripts/phase-runner.sh --plan interact phase-0 --pr --wait
```
...
```bash
scripts/phase-runner.sh --plan interact --from 3 --to 6 --pr --wait
```
