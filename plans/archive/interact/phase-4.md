...
- Add bounded `interact lab export` and `interact lab import` commands requiring the current opaque
  `sessionId`, with an explicit artifact kind of `setup` or `replay`. Store artifacts and alias
  sidecars under the current worktree's ignored `target/interact/lab/` root.
...
- If implementation evidence supports a safer equally bounded local transport, document the
  ...
  request order.
- Persist aliases in a small Interact sidecar keyed to artifact identity, never inside protocol
...
- Add `interact lab artifact-inspect` to return authoring metadata, map, tick/duration,
...
- Let `open` or `import` select an artifact only by opaque id or a path confined beneath the current
  worktree's `target/interact/lab/`. Reject URLs and arbitrary filesystem paths.
...
- Interact daemon/driver artifact store, CLI schemas, and path confinement
...
- Keep every file operation confined to `target/interact/lab/` and every response bounded.
