# Phase 4 - Directly Executable TypeScript
...
- Add TypeScript 5.8 or newer and Node typings to the repository-owned dependency manifest/lock from
  Phase 3. Do not commit emitted JavaScript, declarations, source maps, or `dist/`.
...
- Teach `scripts/check-source-file-sizes.mjs`, the architecture checker, and
...
- The deep rename or another architecture redesign.

## Handoff Expectations

Report the Node minimum, dependency/config/check commands, modules converted, remaining JavaScript
boundary, runtime validators preserved, and any localized type assertions. Provide final checkpoint
evidence for the live semantic smoke, direct no-build startup, architecture ratchets, and manual
