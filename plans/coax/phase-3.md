...
- Stale entities, missing combat state, unknown weapon ids, and zero-damage profiles are safe
  no-ops.
- A Tank can later hold separate `tank_cannon` and `tank_coax` cooldown entries without one
...
Name the cooldown storage shape and the exact API Phase 7 should use for `tank_coax`. Call out
which compatibility shims remain and which callers still need migration before they can use
non-default weapons.
