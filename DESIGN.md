...
- **Rifleman Charge**: after completing a Training Centre, selected riflemen gain a `charge`
  command on the command card's `Z` slot. The command sets `RIFLEMAN_CHARGE_TICKS` (128 ticks) on
  owned riflemen only; while active, movement uses `RIFLEMAN_CHARGE_SPEED_MULTIPLIER` (1.5x) and
  the timer decays every simulation tick, even while idle. Charged riflemen can fire while moving,
  with the charge-fire miss chance described above. The duration is intentionally hard-coded from
  the current "six tank lengths" feel and does not reference tank body constants.
